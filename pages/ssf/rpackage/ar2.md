...
of size $r=max(p,h+1)$, where $h$ is the forecast horizon
desired by the user. If independently of the AR order the user specifies a number of lagged values required ( $nlags$), 
then the size of the state vector will be $r=nlags+h$.
