...
The SSF corresponding to this model, including the transition matrix $T_t$ and the covariance of the transition innovations $V_t$, are defined [here](../implementations/ll.html) 
...
The following R code defines a local level model the variance of which ( $$ \sigma^2_l $$ ) will be a free parameter (`fixed=FALSE`) that will set equal to one (`variance=1`) only 
for the purpose of initializing the estimation algorithm. The default argument `initial=NaN` will trigger 
the algorithm for [diffuse initialization](../algorithms/dk.html).

### jd3_ssf_locallevel
***
R users can define local level model that belongs to the JD+ class `JD3_SsfItem`
...
<div class="tg-wrap"><table id="tg-rBYYg" class="tg">
  ...
  <tr>
    ...
    <td class="tg-0pky"> <i>logical</i>  that triggers estimation of $\sigma^2_l$ (<b>FALSE</b>) or  fixes it (<b>TRUE</b>) to the value set by the parameter variance </td>
  ...
  <tr>
    ...
    <td class="tg-0pky">Default  <b>NaN</b>  triggers the diffuse KF </td>
  </tr>
</table></div>
...
```
jd3_ssf_locallevel<-function(name, variance=1, fixed=FALSE, initial=NaN){
...
The default measurement equation that links a measurement $$ y_{t} $$  of the latent variable $$l_t$$, which is introduced in the next point,
is expressed as:
...
This is the R function that defines this measurement equation 
```
jd3_ssf_equation<-function(name, variance=0, fixed=TRUE){
    jrslt<-.jnew("demetra/msts/ModelEquation", name, variance, fixed)
  new (Class = "JD3_SsfEquation", internal = jrslt)
}
```
where the default `variance=0`, `fixed=TRUE` implies that $$\sigma^2_{\epsilon}=0$$ 

**Add latent variables to the measurement equation**:

Adding the term $$l_t$$ to our equation is done by using the following method
```
...
```
where one needs to specify the name of the equation and the name of the factor $$l_t$$. The default `coeff=1` and `fixed=TRUE` define a coefficient equal to 1. The option  `loading=NULL` simply links $$y_t$$ with $$l_t$$ contemporaneously. 
The example below shows how it works in practice.

### Simple example

In this example we define two states: one local level model with default specification settings (i.e. random walk) and a degenerate state that 
always takes the same constant value (i.e. a constant parameter) 
...
- Second, we add our random walk and name it as "trend". In the absence of further arguments, this function triggers the default specification mentioned above:
```
  add(yourModel, jd3_ssf_locallevel("trend"))
```
- Third, we add our second state variable, which is called "constant". We specify it as a local-level model with a variance  $$ \sigma^2_l=0 $$. Given the inital 
state  $$ l_{0} $$, the model implies $$ l_{t}=l_{0} $$ for all $$t>0$$. 
```
  add(yourModel, jd3_ssf_locallevel("constant", variance = 0, fixed = TRUE)) 
```
 
**Measurement equation design**:

Let's define the following measurement equation 

$$ eq1 :  y_{t} =  c + l_t + \epsilon_{t} $$

where  $$ \epsilon_{t} $$ is I.I.D. with $$ var(\epsilon_{t}) =\sigma^2 \sigma^2_{\epsilon} $$

- Let's assign the name "eq1" to our measurement equation, where the time series called "yourData" ($$ y_{t} $$) is 
assumed to be given the two factors "trend" ($$ \tau $$) and "constant" ($$ c $$). By default, they enter with coefficient equal to 1
...
add(eq1, "constant")                                         
```
-  By default, the coefficient associated to the two factors (constant and trend) is equal to one. In order to relax this assumption
and estimate a different model:
$$ eq1 :  y_{t} =c + \alpha \tau_{t} + \epsilon_{t} $$, 
...
Suppose that instad of only one variable we have two different measurements $$ y_{1,t}, y_{2,t}$$  for the level $$ l_t  $$. The two measurement
equations follow:

$$ eq1 :    y_{1,t} =c_{1} + \alpha_{1} \tau_{t} + \epsilon_{1,t} $$, 

$$ eq2 :    y_{2,t} =c_{2} + \alpha_{2} \tau_{t} + \epsilon_{2,t} $$, 
...
add(newModel, jd3_ssf_locallevel("c2", variance = 0, fixed = TRUE)) 
...
eq2 <-jd3_ssf_equation("y2" ) // by default: (variance=1, fixed = FALSE)                            
add(eq1, "tau", 0.5, FALSE)                                         
add(eq1, "c2")                                         
